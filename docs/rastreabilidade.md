...
|---|---|
| Qualidade do ETL (30%) | `src/etl/` (validacao com quarentena por motivo, limpeza multiformato, full refresh idempotente), `sql/ddl/` (star schema com PKs, FKs, indices), `docs/relatorio_qualidade.md`, `tests/test_limpeza.py`, `tests/test_ordem_views.py` (valida estaticamente a ordem das views, sem banco) |
